...
export const ComparisonContentSection = ({ productIds }: ComparisonContentSectionProps): JSX.Element => {
  ...
  return (
    <section className="w-full bg-white py-[35px]">
      <div className="max-w-7xl mx-auto px-4">
        {/* Header Section */}
        <div className="flex gap-[52px] mb-[48px] overflow-x-auto">
          ...
          {products.map((product, index) => {
            const rating = 4.0 + Math.random() * 1.0; // Generate realistic rating
            const reviews = Math.floor(Math.random() * 300) + 50; // Generate review count
            
            return (
              <div key={product._id} className="w-[280px] flex-shrink-0 relative">
                {/* Remove button */}
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => handleRemoveProduct(product._id)}
                  className="absolute top-2 right-2 z-10 w-8 h-8 p-0 bg-white/80 hover:bg-white rounded-full"
                >
          ...
          {/* Add Product Section */}
          {products.length < 3 && (
            <div className="w-[242px] flex-shrink-0 relative">
              <h3 className="[font-family:'Poppins',Helvetica] font-medium text-black text-2xl mb-[25px]">
                Add A Product
              </h3>
              <Button 
                onClick={() => setShowProductSelector(!showProductSelector)}
                className="w-full h-[39px] bg-[#b88e2f] hover:bg-[#a67d28] text-white [font-family:'Poppins',Helvetica] font-semibold text-sm rounded-md"
              >
                Choose a Product
              </Button>
              ...
              {showProductSelector && (
                <div className="absolute z-20 mt-2 w-[300px] max-h-[300px] overflow-y-auto bg-white border border-gray-200 rounded-md shadow-lg">
                  {allProducts
