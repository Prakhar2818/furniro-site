...
export const ProductGridSection: React.FC<ProductGridSectionProps> = ({ onEditProduct }) => {
  ...
  const handleEditProduct = (e: React.MouseEvent, product: Product) => {
    ...
    if (onEditProduct) {
      // Show edit confirmation modal first
  ...
  const handleEditConfirm = () => {
    if (editModal.product && onEditProduct) {
      onEditProduct(editModal.product);
  ...
  const handleDeleteProduct = (e: React.MouseEvent, productId: string) => {
    ...
    if (product) {
      ...
      setDeleteModal({ isOpen: true, product, isLoading: false });
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await actions.deleteProduct(deleteModal.product._id);
      
      // Show success toast notification
      toast.success(`🗑️ ${deleteModal.product.name} has been successfully deleted!`, {
        position: "top-right",
        autoClose: 3000,
      ...
      toast.error(`Failed to delete ${deleteModal.product.name}. Please try again.`, {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  return (
    <section className="w-full bg-white py-8 sm:py-12 lg:py-16">
            ...
            >
              <CardContent className="p-0">
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 sm:top-4 sm:left-4 flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 z-10">
                    <Button
                      size="icon"
                      variant="secondary"
                      className="w-7 h-7 sm:w-8 sm:h-8 bg-white/90 hover:bg-white shadow-md"
                    ...
                    <Button
                      size="icon"
                      variant="destructive"
                      className="w-7 h-7 sm:w-8 sm:h-8 bg-red-500/90 hover:bg-red-600 shadow-md"
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to delete "${deleteModal.product?.name}"? This action cannot be undone and will permanently remove the product from your inventory.`}
      ...
      <ConfirmationModal
        ...
        title="Edit Product"
        message={`Do you want to edit "${editModal.product?.name}"? You will be able to modify all product details including name, price, description, and stock.`}
